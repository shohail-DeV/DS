...
# Extract Computer Science marks
cs_marks = [student["Computer Science"] for student in student]

# Calculate mean using the statistics library
mean_cs = statistics.mean(cs_marks)

# Calculate median using sorted list indexing (works for even or odd data size)
median_cs = sorted(cs_marks)[len(cs_marks) // 2]


cs_counts = Counter(cs_marks)
mode_cs = cs_counts.most_common(1)[0][0]  # Get the first element (mode) from the most common count

print("\nComputer Science Marks:")
print(f"Mean: {mean_cs:.2f}")
print(f"Median: {median_cs}")
print(f"Mode: {mode_cs}")