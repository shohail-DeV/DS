...
# Extract Math and Science marks
math_marks = [student["Math"] for student in student]
science_marks = [student["Science"] for student in student]

# Calculate the mean for Math and Science marks
mean_math = sum(math_marks) / len(math_marks)
mean_science = sum(science_marks) / len(science_marks)

# Calculate covariance using a loop for clarity
covariance = 0
for i in range(len(student)):
    covariance += (math_marks[i] - mean_math) * (science_marks[i] - mean_science)

covariance /= (len(student) - 1)  # Bessel's correction for unbiased covariance

print("\nCovariance (Math, Science):", covariance)